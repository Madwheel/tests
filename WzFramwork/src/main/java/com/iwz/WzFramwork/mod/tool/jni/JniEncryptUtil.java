...
public class JniEncryptUtil {
    ...
    private static String encrypt(String data, String key) {
        try {
            ...
            //调用C的加密方法，加密数据
            byte[] encryptData = encrypt(base64.getBytes(), base64.length(), key.getBytes(), key.length());
            //返回加密数据的MD5值
     ...
     * @return 加密数据
     */
    public native static byte[] encrypt(byte[] data, int orgLen, byte[] key, int keyLen);
}
